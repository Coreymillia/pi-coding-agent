...
def get_default_pi_agent_project() -> PiAgentProjectManifest:
    projects = list_pi_agent_projects()
    if not projects:
        raise FileNotFoundError("No PiAgent projects exist yet.")
    return projects[0]


def ensure_default_pi_agent_project() -> PiAgentProjectManifest:
    try:
        return get_default_pi_agent_project()
    except FileNotFoundError:
        return create_pi_agent_project(
            "PiAgent Workspace",
            "Default Pi3Groq PiAgent workspace. Use this project when the agent auto-starts on boot.",
        )
...
class PiAgentSessionManager:
    ...
    def _status_unlocked(self) -> dict[str, Any]:
        ...
        return {
            ...
            "displayText": self._display_text,
            "lastOutputAtMs": self._last_output_at_ms or None,
    ...
    def _append_history(self, text: str) -> None:
        with self._lock:
            ...
            plain_text = ANSI_ESCAPE_PATTERN.sub("", text).replace("\r", "")
            if plain_text:
                self._display_text = f"{self._display_text}{plain_text}"[-PI_AGENT_DISPLAY_TEXT_LIMIT:]
                self._last_output_at_ms = int(time.time() * 1000)
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if route == "/api/pi-agent/start":
            try:
                ...
                project = (
                    get_pi_agent_project_or_throw(project_id)
                    if project_id
                    else get_default_pi_agent_project()
                )
        ...
        if route == "/api/pi-agent/input":
            try:
                ...
                if body.get("submit", True):
                    text = f"{text}\n"
                self.server.pi_agent_session.write_input(text)
...
def main() -> None:
    ...
    if pi_agent_bridge.error:
        ...
        if PI_AGENT_AUTOSTART and pi_agent_session.is_available():
            try:
                default_project = ensure_default_pi_agent_project()
                pi_agent_session.start(
                    PI_AGENT_DEFAULT_COLS,
                    PI_AGENT_DEFAULT_ROWS,
                    default_project,
                )
                print(
                    f"[Pi3Groq] Auto-started PiAgent in {default_project.name}"
                )
            except RuntimeError as error:
                print(f"[Pi3Groq] PiAgent autostart skipped: {error}")
